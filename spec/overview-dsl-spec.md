...
The Overview DSL lives inside an `overview <mode> { ... }` block in the scene definition. It is optional. Scenes without it run without structural enforcement.
...
| Element | Form | Required | Description |
|---------|------|----------|-------------|
| mode    | bare identifier after `overview` | yes | Enforcement mode: `nodes_only`, `at_least`, or `strict` |
| body    | `{ ... }` of flow statements | yes | Flow statements, one per line |

- The block body MUST declare at least one statement. An empty body MUST fail with `OverviewFlowEmpty`.
- `enforce` MUST be one of the three enumerated values. Any other value MUST fail with `OverviewInvalidMode`.
- A scene MUST NOT contain more than one `overview` block. A duplicate MUST fail with `OverviewDuplicate`.
- The block is unlabelled, so there is no view name to validate. `SCN_OVERVIEW_UNKNOWN_VIEW` was retired with the label.
...
for each line L in flow.split("\n"):
  ...
  if trimmed.startsWith("|=>"):
    ...
    if target == "":
      fail OverviewEdgeNoTarget
    if current == null:
      fail OverviewEdgeWithoutSource
    ...
    for each part in parts:
      validate IDENT(part)            # fail OverviewInvalidIdent if not IDENT
    ...
    # --- node line ---
    source := trimmed
    validate IDENT(source)            # fail OverviewInvalidIdent if not IDENT
    nodes.add(source)
    current := source
```
...
- `nodes_only`: any `overview_node` not in `impl_nodes` → `OverviewUnknownNode`
- `at_least` (nodes): same as `nodes_only`
- `at_least` (edges): any `overview_data_edge` not in `impl_data_edges` → `OverviewMissingEdge`
- `strict` (nodes): any `overview_node` not in `impl_nodes` → `OverviewUnknownNode`, and any `impl_node` not in `overview_nodes` → `OverviewExtraNode`
- `strict` (edges): any `overview_data_edge` not in `impl_data_edges` → `OverviewMissingEdge`, and any `impl_data_edge` not in `overview_data_edges` → `OverviewExtraEdge`
...
| Compile     | `"overview_compile"`  | Structural errors after parse (e.g. `OverviewInvalidMode`) |
...
| Code                    | Condition |
|-------------------------|-----------|
| `OverviewInvalidMode`   | `enforce` value is not one of the three valid strings |
| `OverviewDuplicate`         | More than one `overview` block in a scene |

### 9.3 Enforcement errors (`stage: "overview_enforce"`)

| Code               | Mode(s)              | Condition |
|--------------------|----------------------|-----------|
| `OverviewUnknownNode` | `nodes_only`, `at_least`, `strict` | `overview_node ∉ impl_nodes` |
| `OverviewExtraNode`   | `strict`             | `impl_node ∉ overview_nodes` |
| `OverviewMissingEdge` | `at_least`, `strict` | `overview_edge ∉ impl_data_edges` |
| `OverviewExtraEdge`   | `strict`             | `impl_edge ∉ overview_data_edges` |
