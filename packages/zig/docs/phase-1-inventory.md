...
| Actions and prepare, merge, and publish ordering | packages/zig/scene-runner/src/action.zig and runner.zig |
| STATE schema checks, unchecked STATE, and snapshots | packages/ts/scene-runner/tests/state-manager.test.ts |
| Next rules, queues, warnings, limits, and partial STATE | packages/zig/scene-runner/src/scene.zig and packages/ts/scene-runner/tests/safe-execution.test.ts |
| Routes, matching, transitions, and limits | packages/zig/scene-runner/src/route.zig and route_vectors.zig |
...
| Binding declaration order and missing compute or root | compute.zig, compute_vectors.zig, and runner.zig |
| STATE reads and asynchronous prepare hooks | runner.zig and runner.test.ts |
| Batched merge writes and schema enforcement | action.zig, state.zig, and state-manager.test.ts |
| Publish order, missing hooks, returned failures, and thrown failures | effect_vectors.zig, runner.zig, and runner.test.ts |
| Merge committed before strict publish failure | runner.zig and safe-execution.test.ts |
...
| Cancellation, limits, and partial STATE | runner.zig, runner.test.ts, and safe-execution.test.ts |
