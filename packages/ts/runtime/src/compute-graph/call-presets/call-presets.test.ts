...
import { buildArray, buildBoolean, buildNumber, buildString, buildNull } from '../../state-control/value-builders';
...
describe('call preset lookup', () => {
  it('resolves binary functions for every namespace', () => {
    expect(getBinaryFn('binaryFnArray::concat' as any)(
      buildArray([buildNumber(1)]),
      buildArray([buildNumber(2)])
    ).value).toHaveLength(2);

    expect(getBinaryFn('binaryFnBoolean::and' as any)(
      buildBoolean(true),
      buildBoolean(false)
    ).value).toBe(false);

    expect(getBinaryFn('binaryFnGeneric::isEqual' as any)(
      buildNumber(1),
      buildNumber(1)
    ).value).toBe(true);

    expect(getBinaryFn('binaryFnNumber::add' as any)(
      buildNumber(2),
      buildNumber(3)
    ).value).toBe(5);

    expect(getBinaryFn('binaryFnString::concat' as any)(
      buildString('turn'),
      buildString('out')
    ).value).toBe('turnout');
  });
  ...
  it('resolves transform functions for every namespace', () => {
    ...
    expect(getTransformFn('transformFnNull::pass' as any)(buildNull('missing')).subSymbol).toBe('missing');
    expect(getTransformFn('transformFnString::trim' as any)(buildString('  ok  ')).value).toBe('ok');
  });
