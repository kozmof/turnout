...
describe('builder value helpers', () => {
  it('builds primitive and null values', () => {
    expect(val.number(7, ['n'])).toEqual({ symbol: 'number', value: 7, subSymbol: undefined, tags: ['n'] });
    expect(val.string('x', ['s'])).toEqual({ symbol: 'string', value: 'x', subSymbol: undefined, tags: ['s'] });
    expect(val.boolean(false, ['b'])).toEqual({ symbol: 'boolean', value: false, subSymbol: undefined, tags: ['b'] });
    expect(val.null()).toEqual({ symbol: 'null', value: null, subSymbol: 'unknown', tags: [] });
    expect(val.null('missing', ['m'])).toEqual({ symbol: 'null', value: null, subSymbol: 'missing', tags: ['m'] });
  });

  it('builds all typed array variants', () => {
    expect(val.array('number', [val.number(1)], ['a'])).toMatchObject({ symbol: 'array', subSymbol: 'number', tags: ['a'] });
    expect(val.array('string', [val.string('x')])).toMatchObject({ symbol: 'array', subSymbol: 'string' });
    expect(val.array('boolean', [val.boolean(true)])).toMatchObject({ symbol: 'array', subSymbol: 'boolean' });
    expect(val.array('null', [val.null('error')])).toMatchObject({ symbol: 'array', subSymbol: 'null' });
  });
  ...
  it('creates reference marker objects', () => {
    ...
    expect(ref.transform(ref.output('sum'), ['transformFnNumber::abs', 'transformFnNumber::toStr'])).toEqual({
      __type: 'transform',
...
describe('runtime utility helpers', () => {
  ...
  it('exposes typed object helpers and namespace delimiter', () => {
    ...
    expect(TOM.entries(obj)).toEqual([['a', 1], ['b', 'two']]);
