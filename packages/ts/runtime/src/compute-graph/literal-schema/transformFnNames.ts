...
const transformFnNumberNames = (): LiteralSchema<TransformFnNumberNames, undefined>[] => {
  const namespace: TransformFnNumberNameSpace = "transformFnNumber";
  const fnNames = TOM.keys(tfNumber);
  return fnNames.map((fnName) => literal(`${namespace}${NAMESPACE_DELIMITER}${fnName}`));
};

const transformFnBooleanNames = (): LiteralSchema<TransformFnBooleanNames, undefined>[] => {
...
const transformFnNullNames = (): LiteralSchema<TransformFnNullNames, undefined>[] => {
