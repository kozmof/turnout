...
function scene(id: string, entry: string, next?: SceneBlock['actions'][number]['next']): SceneBlock {
  return {
    id,
    entryActions: [entry],
...
describe('createRouteStepper', () => {
  ...
  it('exposes partial state after completed actions', async () => {
        ...
        compute: {
          ...
          prog: {
            ...
            bindings: [
              ...
              { name: 'out', type: 'number', expr: { combine: { fn: 'add', args: [{ ref: 'v' }, { lit: 0 }] } } },
            ],
          },
        },
    ...
    expect(stepper.partialState().read('route.value')).toMatchObject({ symbol: 'number', value: 7 });
  });

  it('rejects missing entry and transition scenes', async () => {
    expect(() => createRouteStepper(
      'missing_entry',
      parseMatchArms([]),
      'missing',
      {},
