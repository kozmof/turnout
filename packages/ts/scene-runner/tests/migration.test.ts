...
describe('checkForExtExpr', () => {
  function makeSceneWithExtExpr(inNextRule = false): SceneBlock {
    ...
    const action = inNextRule
      ? {
          id: 'act',
          compute: { root: 'y', prog: { name: 'main', bindings: [{ name: 'y', type: 'number', value: 1 }] } },
      ...
      scenes: [
        {
          id: 'clean_scene',
          entryActions: ['a'],
          actions: [{ id: 'a', compute: { root: 'v', prog: { name: 'p', bindings: [{ name: 'v', type: 'number', value: 42 }] } } }],
        },
      ],
      routes: [],
    } as unknown as TurnModel;
  ...
  it('createSceneRunner throws synchronously before execution when a scene has extExpr', () => {
    ...
    expect(() =>
      createSceneRunner(scene, { initialState: {} }, stateManagerFromUnchecked({})),
    ).toThrow('extExpr');
  });
