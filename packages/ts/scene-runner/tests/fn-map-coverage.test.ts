...
import { getCombineFnReturnType } from "runtime";
...
describe("FN_MAP coverage", () => {
  ...
  it("all non-array FN_MAP values are valid runtime CombineFnNames", () => {
    for (const [hclName, runtimeName] of Object.entries(FN_MAP)) {
      if (runtimeName.startsWith("combineFnArray::")) continue;
      const returnType = getCombineFnReturnType(runtimeName as CombineFnNames);
      expect(
        returnType,
        `FN_MAP["${hclName}"] = "${runtimeName}" is not a known runtime CombineFnName`,
      ).not.toBeNull();
    }
  });

  it("all array FN_MAP values follow the combineFnArray:: namespace pattern", () => {
    const arrayFns = ["arr_concat", "arr_get", "arr_includes"] as const;
    for (const name of arrayFns) {
      expect(FN_MAP[name], `FN_MAP["${name}"] should start with "combineFnArray::"`).toMatch(
        /^combineFnArray::/,
      );
    }
  });
