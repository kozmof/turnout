...
thing. So the whole knob goes, taking with it the keyword, AST field, proto field,
TypeScript type, and the diagnostic that exists only to police the interaction between
`all-match` and match blocks.
...
- `NextMatchPolicy` (`diag_codes.go:157`) and `checkMatchPolicy`
  (`parser_scene.go:199-222`). A match block requires first-match, and first-match is now the
...
`reserved 3; reserved "next_policy";` to `SceneBlock`. Reserving matters, because field 3 must never
...
`overview_test.go:203` (`TestEnforceNodesOnlyAllMatch`) is unrelated, since "all match" there means
all overview nodes matched. Leave it, and consider renaming to avoid the collision when grepping.
...
- `run-state.ts` — `enqueueNext` drops its `policy` parameter, and each of the two duplicate
...
- `SceneRuntimeError("UnsupportedConstruct", …)` for a bad policy string disappears with
  `next-policy.ts`. Nothing replaces it, because an unknown attribute is now a converter-side parse
...
warning still fires, since two different actions can each name the same target, so the fixtures
...
match-block/policy rule), 78 and 90 (`nextPolicy` in the runtime data model, noting that line 90 is
...
`spec/hook-spec.md:74,246,260` say "all matching bindings", which is unrelated prose. Leave it alone.
...
to both `file_report` and `schedule_service` (161). Decision taken. Rewrite it as the first-match
...
- A JSON model still carrying `"nextPolicy"` fails `bridge.ts` strict parsing and passes
  lenient, so assert both, since this is the only externally visible break.
...
ran" and "already pending". The pending one is now unreachable, since selection returns at most one
...
`halved`). That predates this change, since the compute blocks were not touched, and the e2e suite
