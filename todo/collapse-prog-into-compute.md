...
`next { action = b }` with no compute at all is unaffected, as it is already the
...
- Delete both duplicate-`prog` guards (`:287` and `:654`). They become
  unreachable, since with no inner block there is nothing to repeat. This removes
...
`CodeDuplicateProg` stays defined, because `hcl-context-spec.md:564,747` still uses it
...
files, the `.tu` input compiled via `CompileSource`, and expected canonical HCL
...
- `convert-runtime-spec.md:46` claims the CLI emits `prog "<actionId>"`, but
  ...
  name is author-chosen, and the decision above makes that binding.
...
1. `TMPDIR=/workspace/tmp go test ./...` from `packages/go/converter`, plus
   ...
   environment, and the Go toolchain fails before running anything without it.)
...
3. New `TestParseLegacyProgBlock` in `parser_irregular_test.go`, replacing
   ...
   diagnostics only, since `ParseFile` returns a nil `*ast.TurnFile` whenever there
...
in step 5 of the implementation, a wide and mostly mechanical migration across
